"""Use Pi to create one immutable Git-backed challenger artifact.

This bridge is outside the six-app control plane. It exposes no protected cases
or evaluator results. It is not an OS sandbox; run it inside a reviewed isolated
builder before using untrusted models or repositories.
"""
...
class ProposerError(RuntimeError):
    """Raised when Pi does not produce one valid Git challenger."""
...
def _pi_prompt(context: dict[str, object], artifact: dict[str, object]) -> str:
    return (
...
def _run_pi(workspace: Path, prompt: str) -> None:
    command = [
...
def _commit_challenger(
    ...
    if not paths:
        raise ProposerError("Pi and the build command did not change the candidate")
    ...
    published_ref = f"{ref_prefix.rstrip('/')}/generation-{generation:06d}-{commit[:12]}"
...
def propose(source: str) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="metering-git-proposer-") as temporary:
        ...
        _run_pi(workspace, _pi_prompt(context, parent))
        _validate(workspace)
    ...
    return {
        ...
        "reason": (
            f"Pi produced immutable Git commit {challenger['commit']} "
            f"at {published_ref}"
        ),
    }
...
def main() -> int:
    try:
        response = propose(sys.stdin.read())
    except (
        GitCandidateError,
        ProposerError,
        ProtocolError,

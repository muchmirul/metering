...
Folder identity is a separate layer. Canonical manifests and SHA-256 produce a
content `tree_id`; hashing that ID with the parent snapshot ID produces a
lineage-specific `snapshot_id`. Those hashes identify content and ancestry.
They do not supply probabilities, authenticate an author, or justify the probe
policy.
...
1. **Closed world:** the active sandbox is exactly one registered fixture.
...
file-manifest hash to equal the selected version's `tree_id`. Thus extra or
changed files reject the closed-world hypothesis instead of being silently
ignored.
...
No alternate scoring algorithm improves this four-version example. A global
decision-tree optimizer returns the same two-read tree but adds complexity; a
separate mutual-information computation duplicates result entropy under the
deterministic channel. The narrow useful refinement is to document the model
and pin its identities and predictions in tests, while keeping the production
loop unchanged.

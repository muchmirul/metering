...
| **Pi** | Provides one streamlined `/agentvolve` workflow UI with an activation-scoped cross-session `[1/6]`–`[6/6]` monitor, a shared `/agentvolve-history` browser, and compatibility `/evolve-*` commands. The launcher offers local Qwen/llama.cpp or retains the preceding routed Pi model. Nested calls stay manifest-pinned, use isolated configuration, and disable sessions, skills, extensions, prompt templates, context files, and provider tools. | Sandbox enforcement, evaluator authority, Population authority, automatic installation, or silently substituting the routed UI model for the experiment model. |

...
| **Pi** | Interactive clarification/review UI for delegated jobs, never a mode. Configured tools/model/thinking stay unchanged; old mode records impose no restrictions. Four commands remain (`/goal`, `/limit`, `/history`, `/progress`). Each job needs a fresh exact cap, explicit compatible seal and separate worker runtime/configuration review. Submission records bind only acknowledged job IDs/roots; the widget, progress and verification never follow newest unrelated runs. History inspection does not rebind; resume does not restart; forks inherit no ownership. Detached worker calls remain manifest-pinned and isolated from sessions, skills, extensions, templates, context files and provider tools. | Worker execution, host sandbox enforcement, evaluator/Population authority, automatic controller installation, shared-service restart, implicit Level-2 setup or substituting the interactive model for the worker. |

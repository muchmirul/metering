...
| **Pi** | Interactive clarification and typed configuration for delegated jobs, never a mode. Configured tools/model/thinking stay unchanged; old mode records impose no restrictions. Five commands remain (`/goal`, `/limit`, `/history`, `/progress`, `/agentvolve-stop`). Complete validated task/setup fields queue without modal approval; missing fields/options are returned for conversational collection. Submission records bind only acknowledged job IDs/roots; the widget, progress, stop, and verification never follow newest unrelated runs. History inspection does not rebind; resume does not restart; forks inherit no ownership. Detached worker calls remain manifest-pinned and isolated from sessions, skills, extensions, templates, context files and provider tools. | Worker execution, host sandbox enforcement, evaluator/Population authority, automatic controller installation, shared-service restart, implicit Level-2 setup, inventing user settings, or substituting the interactive model for the worker. |
| **Trace Viewer** | Explicitly opened local candidate/file graph with stable branch labels, exact Git inspection, captured public evidence, history/comparison, and exports. Keeps the terminal fallback. | Evolution, source execution, archive edits, protected inputs, full offline verification, or persistent candidate storage. |
